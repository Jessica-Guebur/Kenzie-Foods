function createCardFood(list){
    ...
    btnAddFood.append(spanAddFood);

    // ADIOMAR CONTEUDOS INTERNOS 
    imgFood.src = list.img;
    imgFood.alt = `Imagem do prato ${list.name}`;
    h3FoodTitle.innerText = list.name;
    pFoodPrice.innerText = `R$ ${list.price.toFixed(2)}`;
    spanAddFood.innerText = "Adicionar";

    // ADICIONANDO AS CLASSES 

    liFood.classList.add("food_item");
...
function renderCards(foodList){
    ...
    console.log(ulMenuFoodList);

    foodList.forEach((food, index) => {
        const foodCard = createCardFood(food);
